...
max_heap_node_ptr Create_Max_Heap_Node(int outbreaks, char* name)
{
...
void Max_Heap_Insert(max_heap_ptr heap, int outbreaks, char* name)
{
...
{ 
    if (root == NULL) 
        return; 
    if (level == 1) 
        printf("%d %s ",root->data->outbreaks ,root->data->name); 
    else if (level > 1) 
    { 
...
void Top_k(max_heap_ptr heap, int k)
{
    int i;
